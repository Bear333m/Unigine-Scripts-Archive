        ...
        public void CreateCircleScen()
        {
            ...
            if (!Storage.AuxiliaryPlane_1_Bool)
            {
                if (Input.IsMouseButtonDown(Input.MOUSE_BUTTON.LEFT))
                {
                    ...
                    //Storage.sizePng = new vec2(0.50f, 0.50f);
            ...
            else
            {
                ...
                Node.SetPhysicalSize(MathLib.Max(size) * 2, MathLib.Max(size) * 2);
        

                //var Node = World.GetNodeByID(Storage.IDGui);
                //Node.Scale = new vec3(MathLib.Max(size));

                if (Input.IsMouseButtonDown(Input.MOUSE_BUTTON.LEFT))
                {

                    //GUI.Name = "DrawingCO4A7BRrcc3sUsGCO8D7x";
                    Storage.IDGui = 0;
